...
import { formatCitation } from '../../../lib/format-citation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    let ragContext = '';
    let sources: Array<{ slug: string; chunk_index: number; citation: string }> = [];
    try {
        ...
        ragContext = '\n\n**COHERENCEISM KNOWLEDGE BASE:**\n' + 
          rows.map((row, idx) => {
            sources.push({
              ...
              citation: formatCitation({ slug: row.slug, chunk_index: row.chunk_index })
            });
