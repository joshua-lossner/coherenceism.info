...
const ECHOTerminal = () => {
  ...
  const typeResponse = async (
    ...
    sources: Array<{ slug: string; chunk_index: number; citation?: string }> = []
  ) => {
    ...
    if (sources.length > 0) {
      const citationLine = sources.map(s => s.citation || `${s.slug}#${s.chunk_index}`).join('; ')
  ...
  const callOpenAI = async (
    message: string,
    mode: 'query' | 'conversation' = 'conversation',
    clearContext: boolean = false
  ): Promise<{ response: string; sources: Array<{ slug: string; chunk_index: number; citation?: string }> } | null> => {
    try {
